...
function ks = calculateKS(CN, siz, method)
    % CN = digraph.
    % siz = image size.
    ...
    switch(method)
        case 1 
            % Calculates the sum of weight of the outgoing edges.
            C = centrality(CN, 'outdegree', 'Importance', CN.Edges.Weight);
        case 2
            % The 'incloseness' computes the inverse sum of the distance from a
            % node to all other nodes in the graph.
            C = centrality(CN, 'incloseness', 'Cost', CN.Edges.Weight);
        case 3
